...
if (numRounds === 0) {
  console.error('Ingen "Vækst Runde N"-kolonner fundet i CSV-filen.');
  process.exit(1);
}

const round1 = optimizeSquad(players, 0, p => p.startpris, BUDGET);

// timeline[r-1] = trup holdt i runde r (starter som runde-1-holdet i alle runder,
// og opdateres når et skift vælges).
let timeline = [];
for (let i = 0; i < numRounds; i++) timeline.push(round1.selected.slice());

const swapLog = []; // { t, out, cand, fee, benefit }
const MAX_SWAPS = 3;

function cashTrajectory() {
  // Kontant-saldo EFTER hver rundes bankrente, ud fra nuværende swapLog.
...
function findBestSwap() {
  ...
  for (let t = 2; t <= numRounds; t++) {
    ...
    const cashAtT = trace[t - 2]; // kontant efter forrige rundes rente
...
for (let r = 1; r <= numRounds; r++) {
  ...
  console.log(`\nRunde ${r}${r === 1 ? ` (${round1.formation})` : ''}:`);
  const swapsHere = swapLog.filter(s => s.t === r);
  for (const sw of swapsHere) {
  ...
  console.log(`  Rundescore: ${fmtKr(roundScore)}   Gebyrer: ${fmtKr(feesThisRound)}   Netto: ${fmtKr(netto)}   Kontant: ${fmtKr(finalCash[vIdx])}`);
  console.log('  Trup: ' + squad.map(p => `${p.navn} (${p.hold})`).join(', '));
}
